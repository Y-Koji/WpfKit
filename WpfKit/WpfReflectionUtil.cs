...
namespace WpfKit
{
    public static class WpfReflectionUtil
    {
        ...
        public static EventInfo FindEvent(Type type, string eventName, Type handlerType)
            => type.GetEvent(eventName) is EventInfo @event && @event.EventHandlerType == handlerType ? @event :
                null != type.BaseType ? FindEvent(type.BaseType, eventName, handlerType) :
                null;
        ...
        public static void PropertyChangedHandler(object instance, string propName)
        {
            ...
            {
                var handler = instance.GetType().BaseType
                    .GetField(nameof(INotifyPropertyChanged.PropertyChanged), (BindingFlags)0xFF)
                    .GetValue(instance) as PropertyChangedEventHandler;

                handler.Invoke(instance, new PropertyChangedEventArgs(propName));
            }
        }
        ...
        public static object HandleProperty(object value, MethodInfo handler)
        {
            ...
            if (null == FindEvent(type, nameof(INotifyPropertyChanged.PropertyChanged), typeof(PropertyChangedEventHandler)))
            {
                // めんでぃーからthrow
            ...
            foreach (var method in methods)
            {
                ...
                Debug.WriteLine(
                    $"INotifyPropertyChanged, Auto Implemented: {type.FullName}#{method.Name}\r\n" +
                    $"  -> {dynamicName}#{method.Name}");
            }
